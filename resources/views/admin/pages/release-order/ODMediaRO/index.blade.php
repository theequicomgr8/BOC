...
<div class="content-inside p-3">
  <div class="card shadow mb-4">
    ...
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
      <h6 class="m-0 font-weight-normal text-primary"><i class="fa fa-edit" aria-hidden="true"></i> Outdoor Media RO cum Bill</h6> 
    </div>
    <!-- Card Body -->
    <div class="card-body">
    ...
    <table class="table table-striped table-bordered table-hover order-list" id="myTable">
      ...
      <tbody>
       ...
       <tr>
        ...
        <td>{{$result->RoCode}} </td>
        <td>{{$result->RoCode}} </td>
        <td>{{date('Y-m-d', strtotime($result->PublishDate))}} </td>
        <td>{{ date('Y-m-d', strtotime($result->PublishDate)) }} </td>     
<?php
