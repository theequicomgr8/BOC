<div class="row">
  ...
  <div class="col-md-4">
    <div class="form-group">
      ...
      <input {{ $disabled }} type="text" class="form-control form-control-sm {{$disabled=='disabled' ? 'remove-arrow-select':'' }}" name="crsTentative_budget" maxlength="20" id="crsTentative_budget" placeholder="Enter Tentative Budget." onkeypress="return onlyNumberKey(event)" value="{{round(@$crsRadiocr->{'Budget Amount'}) && round(@$crsRadiocr->{'Budget Amount'})!=0 ? round(@$crsRadiocr->{'Budget Amount'}) :''}}">
    </div>
  </div><!--------end Tentative--------->
    <div class="col-md-4" id="crsRadioSpecificRegionDiv" style="display: none;">
      <div class="form-group">
        ...
        <select {{$disabled }} name="crsRadioRegion[]" id="crsRadioSpecificRegion"  class="form-control form-control-sm {{$disabled=='disabled' ? 'remove-arrow-select':'' }}">
    ...
    <div class="col-md-4" id="crsRadioGroupRegionDiv" style="display: none;">
      <div class="form-group">
        ...
        <select {{$disabled }} name="crsRadioRegion[]" id="crsRadioGroupRegion"  class="form-control form-control-sm {{$disabled=='disabled' ? 'remove-arrow-select':'' }}" multiple>
         @foreach($regionalLang as $regionLang)
          <option value="{{$regionLang->Code}}" {{ 
          in_array( $regionLang->Code, str_replace(' ', '', $radioLangSelectionData )) ? 'selected' : '' }}>{{$regionLang->Code}} ~ {{$regionLang->Name}}</option>
          @endforeach
