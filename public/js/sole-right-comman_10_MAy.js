...
$(document).ready(function () {
  ...
  $("#add_row_next").click(function () {
    ...
    var html =
      ...
      i +
      '" class="form-control form-control-sm" maxlength="14"></p></div></div><div class="col-md-6"></div><div class="col-md-12"><button class="btn btn-danger remove_row_next" data="' + i + '" style="margin: 8% 0 0 91%; font-size: 13px;"><i class="fa fa-minus"></i> Remove</button></div></div>';
  ...
  $(".mediasub").change(function () {
    ...
    if (subcatvalue != '' && subcattext != '') {
      $.ajax({
        ...
        type: 'POST',
        url: '/get-media-excel',
        data: { subcatvalue: subcatvalue, subcattext: subcattext, cattext: cattext },
        success: function (response) {
          console.log(response);
          $("#viewexcel").show();
