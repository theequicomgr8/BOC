...
$(document).ready(function(){
  ...
  var outdoorMediaSubCategoryID = $('#hiddensubcatid').val();
  var outdoorMediaCategory = $('#outdoorMediaCategory option:selected').val();
  if (outdoorMediaSubCategoryID != '') {
    var getmediaurl='getODMediaSubCat/'+outdoorMediaCategory+'/'+ outdoorMediaSubCategoryID
      $.ajax({
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token1"]').attr('content')
        },
        ...
        success: function(response) {
            $('#outdoorMediaSubCategory').append('<option value="' + response[0].ODMediaUID + '" selected ="selected">' + response[0].Name + ' </option>');
          
        },
      });
      //y$('#outdoorMediaSubCategory').html('<option value="">Select media sub Catogry</option>');
    }else{
      $('#outdoorMediaSubCategory').val('');
    }
  $('#outdoorMediaCategory').change(function(){
    var outdoorMediaCategory = $('#outdoorMediaCategory option:selected').val();
    if (outdoorMediaCategory != '') {
        $.ajax({
          headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token1"]').attr('content')
          },
          type: 'GET',
          url: 'getODMediaSubCat/'+outdoorMediaCategory+'/'+'',
          success: function(response) {
            $.each(response, function(index, value) {
              $('#outdoorMediaSubCategory').append('<option value="' + value.ODMediaUID + '">' + value.Name + ' </option>');
            });
          },
        });
        $('#outdoorMediaSubCategory').html('<option value="">Select media sub Catogry</option>');
      }else{
        $('#outdoorMediaSubCategory').val('');
      }
