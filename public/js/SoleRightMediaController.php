...
class SoleRightMediaController extends Controller
{
    ...
    public function soleRightSaveVendorData(Request $request)
    {
        ...
        $find_odmedia=DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID')->where($fwhere)->orderBy('OD Media ID','desc')->first(); //15 feb sk
        // dd($find_odmedia);
        // $check_vendor = DB::table($Check_vendor_table)->select('OD Media ID')->where('OD Media ID', $request->vendorid_tab_2)->first(); 15 feb sk
        // dd($check_vendor);
        ...
        if ($find_odmedia == '' || $find_odmedia == null) {
            ...
            if ($check_owner_detail == '' || $check_owner_detail == null) {
                ...
                if (count($owner_id) > 0) {
                    foreach ($owner_id as $key => $value) {
                        ...
                        // $sql3 =  DB::insert("insert into $table3([timestamp],[OD Media Type],[OD Media ID],[Owner ID])values(DEFAULT,$od_media_category,'" . $odmedia_id . "','" . $owner_id . "')"); //sk change
            ...
            $Check_media_work_table = 'BOC$OD Media Work Done';
            ...
            if($request->xls2==0)
            {
                if ($check_media_work == '' || $check_media_work == null) {
                    ...
                    if(!empty($request->ODMFO_Year[0])){
                        ...
                        //         $fileUploaded = $file->move(public_path() . '/uploads/sole-right-media/', $fileName);
                        //         if ($fileUploaded) {
            ...
            $Check_sole_media_table = 'BOC$Sole Medias Address';
                    ...
                    // if (count($request->MA_City) > 0) {
                        ...
                        foreach ($request->MA_City as $key => $value) {
                            ...
                            if (!empty($request->MA_City[0])) {
                                ...
                                $sql5 = DB::insert("insert into $table5([timestamp],[OD Media Type],[Sole Media ID],[Line No_],[City],[State], [District],[Zone],[Latitude],[Longitde],[Landmark],[Image File Name],[OD Media ID],[Display Size],[Illumination Type],[Availability Start Date],[Availability End Date],[Length],[Width],[Total Area],[Rental],[Rental Type],[Quantity]) values(DEFAULT,$Applying_For_OD_Media_Type,'" . $odmedia_id . "',$line_no,'" . $MA_City . "','" . $MA_State . "','" . $MA_District . "',$MA_Zone,$MA_Latitude,$MA_Longitude,'" . $MA_Property_Landmark . "','" . $Image_File_Name . "','" . $od_media_type . "','" . $ODMFO_Display_Size_Of_Media . "','" . $Illumination_media . "','" . $av_start_date . "','" . $av_end_date . "',0,0,0,0,0,0)");
            ...
            $ODMFO_Display_Size_Of_Media =
                isset($request->ODMFO_Display_Size_Of_Media) ? $request->ODMFO_Display_Size_Of_Media : '';
            // $Illumination_media =
            ...
            //     try {
            //         // DB::table('BOC$OD Media Work done')->where($whee)->delete();
            //         Excel::import(new MediaExcelsImportDone, request()->file('media_import2')); //for import
        ...
        { //update query
            ...
            @$find_odmedia=DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID')->where($f_where)->orderBy('OD Media ID','desc')->first(); //15 feb sk
            ...
            // if (count($request->ODMFO_Year) > 0 ) { comment by sk 12-Feb becouse always count 1 when import excel
                $workcheck = DB::table('BOC$OD Media Work done')->where('OD Media ID', $odmedia_id)->first();
                if ($workcheck) {
                    DB::table('BOC$Sole Medias Address')->where('OD Media ID', $odmedia_id)->delete();
                }
            ...
            DB::table('BOC$OD Auth Representative')->where($where)->delete();
            foreach($request->Authorized_Rep_Name as $key => $rep_name)
            {
                $line_no = DB::table('BOC$OD Auth Representative')->select('Line No_')->where('OD Media ID', $odmedia_id)->orderBy('Line No_', 'desc')->first();
                if (empty($line_no)) {
            ...
            //     DB::table('BOC$OD Media Work done')->where($wheer)->delete();
            // }
