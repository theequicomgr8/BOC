    ...
    public function dashboard()
    {
        ...
        if(Session::has('WingType') == 3)
        {
            if (Session::has('UserName')) {
                $data = DB::table('BOC$Print Vendor Detail$3f88596c-e20d-438c-a694-309eb14559b2')->select('OS Amount')->where('NP Code', session("UserName"))->first();
                if (!empty($data)) {
                    $price = round($data->{'OS Amount'}, 2);
                }
            }
            return view('admin.pages.dashboard',compact('price'));
        }
        else
        {
