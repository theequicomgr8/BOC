...
class AgreementController extends Controller
{
    ...
    {
        $user_id = Session::get('UserID');        
        $table = '[CRONUS India Ltd_$Vend Emp Website$3f88596c-e20d-438c-a694-309eb14559b2]';
        $response = DB::select("select [Agr File Path],[Agr File Name] from $table where [User ID] = '" . $user_id ."' ");
        //print_r($response);die;
        if(count($response) > 0)
        {
        ...
        return view('admin.pages.intWeb-file-upload',$dbresponse);
    }
    ...
    {
      $user_id = Session::get('UserID');        
        $table = '[CRONUS India Ltd_$Vend Emp Digital Cinema$3f88596c-e20d-438c-a694-309eb14559b2]';
        $response = DB::select("select [Agreement File Name] from $table where [User ID] = '" . $user_id ."' And [Empanelment Category]='7' ");
        // print_r($response);die;
        if(count($response) > 0)
        {
...
//Digital Agreement file upload
    public function digital_file_upload(Request $request)
    {
        ...
        $table = '[CRONUS India Ltd_$Vend Emp Digital Cinema$3f88596c-e20d-438c-a694-309eb14559b2]';
        $sql =  DB::update("update $table set  [Agreement File Name] = '" . $fileName . "' where [User ID] = '" . $user_id . "' And [OD Category]='7' ");
        if($sql){
            return back()->with('success','You have successfully upload file!');
        }else{
            return back()->with('failed','Some error occured!');  
    ...
    public function Digital_download_file()
    {
     ...
     {
        $user_id = Session::get('UserID');
        $table = '[BOC$Vend Emp - AV Producer]';
        $response = DB::select("select [Agr File Path],[Agr File Name] from $table where [User ID] = '" . $user_id ."' ");
        if(count($response) > 0)
        {
            $fileName = $response[0]->{'Agr File Name'};
    ...
    public function avProducer_download_file()
    {
     ...
     {
        $user_id = Session::get('UserID');
        $table = '[BOC$Vend Emp - Community Radio]';
        $response = DB::select("select [Agr File Path],[Agr File Name] from $table where [User ID] = '" . $user_id ."' ");
        if(count($response) > 0)
        {
            $fileName = $response[0]->{'Agr File Name'};
