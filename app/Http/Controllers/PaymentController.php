...
class PaymentController extends Controller
{
    ...
    public function index($media_id)
    {
        if (!Session::has('id')) {
            ...
            $owner_id = DB::table('BOC$OD Media Owner Detail')->select('Owner ID')->where('OD Media ID', session('payment_odmediaid'))->first();
            $owner_data = '';
            if (!empty($owner_id)) {
                $owner_data = DB::table('BOC$Owner')->select('Owner Name', 'Mobile No_', 'Email ID', 'Address 1', 'State', 'City')->where('Owner ID', $owner_id->{'Owner ID'})->first();
            }
    ...
    public function verifySignature(Request $request)
    {
            ...
            if ($res === 1) {
                ...
                if ($res == true) {
                    return Redirect('/sole-right-list')->with('msg_success', 'Payment transacstion successful!');
                } else {
                    return Redirect('/sole-right-list')->with('msg_error', 'Some error occurred!');
                }
    ...
    public function insertData($data)
    {
        ...
        $sql = DB::table('BOC$Vendor Fees')->insert($insert_array);
        if ($sql) {
            return true;
        } else {
            return false;
    ...
    {
        $sql = DB::table('BOC$Vendor Fees')->where($where)->update($resData);
        if ($sql) {
            return true;
        } else {
            return false;
    ...
    public function getPaymentDetails(Request $request)
    {
        ...
        $data = DB::table('BOC$Vendor Fees')->select($select)->where('Media ID', $request->od_media_id)->first();
