...
class PersonalMediaController extends Controller
{
    ...
    public function RateSettlPersonalMediaSave(Request $request)
    {
        ...
        $checkOwner=DB::table('BOC$Owner')->select('*')->where('Email ID',$request->owner_email)->first();
    ...
    public function personalRightSaveVendorData(Request $request)
    {
        ...
        $ownerCheck=DB::table('BOC$Owner')->select('*')->where('User ID',$userid)->first();
        // dd($ownerCheck);
        ...
        $od_media_category = 1;
        $Check_vendor_table = 'BOC$Vendor Emp - OD Media';
        ...
        $find_odmedia=DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID')->where($fwhere)->orderBy('OD Media ID','desc')->first();
        ...
        if ($find_odmedia == '' || $find_odmedia == null) {
            ...
            $Illumination_media =isset($request->Illumination_media) ? $request->Illumination_media : 0;
            ...
            $file_name_status = 0;
            $file_check = DB::table('BOC$Vendor Emp - OD Media')->where('OD Media ID', $odmedia_id)->first();
            $destinationPath = public_path() . '/uploads/personal-media/';
            ...
            if($request->boradio==1)
            {
                if($request->BO_Address!='')
                {
                    foreach($request->BO_Address as $key => $branch_address)
                    {
                        ...
                        $branch=DB::table('BOC$OD Branch Offices')->insert($branch_data);
                    }
                    if (!$branch) {
            ...
            $sq12=DB::table('BOC$Vendor Emp - OD Media')->insert($vendorArray);
            ...
            $sq13=DB::table('BOC$OD Media Owner Detail')->insert($detailArray);
            ...
            if($request->xls2==0)
            {
                if (count($request->ODMFO_Year) > 0) 
                {
                    ...
                    foreach ($request->ODMFO_Year as $key => $value) 
                    {
                        ...
                        $sql4=DB::table('BOC$OD Media Work done')->insert($workArray);
                        //  $lineno2[] = $line_no;
            ...
            @$find_odmedia=DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID')->where($f_where)->orderBy('OD Media ID','desc')->first(); 
            ...
            //sole media 
            $table5 = '[BOC$Sole Medias Address]'; 
            $check = DB::table('BOC$Sole Medias Address')->select('*')->where('Sole Media ID', $odmedia_id)->first();
            if ($check) 
            {
            ...
            if(!empty($request->Applying_For_OD_Media_Type))
            {
             ...
             if($request->boradio==1)
             {
                if($request->BO_Address!='')
                {
                    ...
                    DB::table('BOC$OD Branch Offices')->where($where)->delete();
                    foreach($request->BO_Address as $key => $branch_address)
                    {
                        $line_no = DB::table('BOC$OD Branch Offices')->select('Line No_')->where('OD Media ID', $odmedia_id)->orderBy('Line No_', 'desc')->first();
                        if (empty($line_no)) 
            ...
            if($request->xls2==0)
            {
                if (count($request->ODMFO_Year) > 0) 
                {
                    ...
                    { //ram code start
                        ...
                        $sq14=DB::table('BOC$OD Media Work done')->insert($workArray2);
                        DB::unprepared('SET ANSI_WARNINGS ON');
                        // }
                    } //ram code end
                }
            ...
            $ownerCheck=DB::table('BOC$Owner')->select('*')->where('User ID',$userid)->orderBy('Owner ID','desc')->first();
            $owner_id=$ownerCheck->{'Owner ID'};
    ...
    public function personalSaveVendorAccount(Request $request)
    {
        ...
        $data=DB::table('BOC$Vendor Emp - OD Media')->where($where)->first();
    ...
    { 
        ...
        if ($odmedia_id != null || $odmedia_id != '' || $odmedia_id != 0) {
            if ($request->allocated_VC != '') {
                $vendor_table = 'BOC$OD Vendor Renewal';
    ...
    //      $aa=  DB::table('BOC$Vendor Emp - OD Media')->where('User ID','=', 'EMPTQ3')->first();
    ...
    public function showDetails($where, $table_media)
    {
        ...
        $orderBy = '';
        $sort = '';
        if ($table2 == 'BOC$OD Vendor Renewal') {
            $orderBy = 'Line No_';
            $sort = 'DESC';
        }
    ...
    {
        $sql = DB::table('BOC$OD Media Work done')->where(['Line No_' => $line_no, 'OD Media ID' => $od_media_id])->delete();
        if ($sql) {
    ...
    {
        $sql = DB::table('BOC$Sole Medias Address')->where(['Line No_' => $line_no, 'Sole Media ID' => $od_media_id])->delete();
        if ($sql) {
            return $this->sendResponse('', 'Data deleted successfully!');
        } else {
            return $this->sendError('Some Error Occurred!.');
    ...
    public function renewalOutdoorSave(Request $request)
    {
        ...
        $od_data = DB::table('BOC$OD Vendor Renewal')->select('Line No_')->where('OD Media ID', $request->vendorid_tab_2)->orderBy('Line No_', 'desc')->first();
        if (empty($od_data)) {
            $od_line_no = 10000;
        } else {
        ...
        $v_data = DB::table('BOC$OD Vendor Renewal')->select('Status', 'Line No_')->where('OD Media ID', $request->vendorid_tab_2)->orderBy('Line No_', 'desc')->first();
        if (empty($v_data) || $v_data->Status == 6) {
            $insertArray['Line No_'] = $od_line_no;
            $insertArray['Status'] = 0;

            $od_sql = DB::table('BOC$OD Vendor Renewal')->Insert($insertArray);
        } else {

            $od_sql = DB::table('BOC$OD Vendor Renewal')->where(['OD Media ID' => $request->vendorid_tab_2, 'Line No_' => $v_data->{'Line No_'}])->update($insertArray);
        }

        $line_array = [];
        if (!empty($request->ODMFO_Year)) {

            $workdonedata = DB::table('BOC$OD Media Work done')->select('Year', 'Line No_', 'File Name')->where([['OD Media ID', '=', $request->vendorid_tab_2], ['Allocated Vendor Code', '=', '']])->get();

            
            foreach ($request->ODMFO_Year as $key => $value) {

                $line_no = DB::table('BOC$OD Media Work done')->select('Line No_')->where('OD Media ID', $request->vendorid_tab_2)->orderBy('Line No_', 'desc')->first();
                ...
                $data = DB::table('BOC$OD Media Work done')->select('Line No_')->where(['Line No_' => @$workdonedata[$key]->{'Line No_'}, 'OD Media ID' => $request->vendorid_tab_2])->first();

                if (empty($data)) {
                    $insertworkdone['Line No_'] = $line_no;
                    $insertworkdone['Allocated Vendor Code'] = '';
                    array_push($line_array, $line_no);
                    $od_sql = DB::table('BOC$OD Media Work done')->Insert($insertworkdone);
                } else {
                    array_push($line_array, @$workdonedata[$key]->{'Line No_'});
                    $od_sql = DB::table('BOC$OD Media Work done')->where(['Line No_' => @$workdonedata[$key]->{'Line No_'}, 'OD Media ID' => $request->vendorid_tab_2])->update($insertworkdone);
                }
            }
        }
    ...
    { //dd($request);
        ...
        $sql1 = ApiFreshEmpanelment::updateAllRecords('BOC$OD Vendor Renewal', $update, $where);
