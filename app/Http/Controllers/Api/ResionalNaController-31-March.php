...
public function ChanalInformation(Request $request){
    $msg ='';
    $WBdata =DB::table('BOC$Owner')->select('Owner ID')->where('Email ID',$request->email)->first();
    $userid =session::get('UserID');
    $Channel = 'BOC$Vendor Emp - Channel';
/*=================================Start Generate Channel ID=================================*/
    $Channel_id = DB::select('select TOP 1 [Channel ID] from [dbo].[BOC$Vendor Emp - Channel] order by [Channel ID] desc');
    if (empty($Channel_id)) {
        $Channel_id = 'CH5OW1';
    } else {
...
/*=================================End Get Channel ID=================================*/ 
    ...
    $Date_of_EMMC =$request->Date_of_EMMC ?? '1900-01-01 00:00:00.000';
    ...
    //Upload document 
    $FetchAllDoc =DB::table('BOC$Vendor Emp - Channel')->where('User ID',$userid)->first();
    ...
    if($request->hasFile('Uplinking_Down_linking')){ 
    $Uplinking_Down_linking =$request->file('Uplinking_Down_linking');
    ...
    if($request->hasFile('EMMC_certificate')){
    $EMMC_certificate =$request->file('EMMC_certificate');
    ...
    if($request->hasFile('Fixed_Point_Chart')){
    $Fixed_Point_Chart =$request->file('Fixed_Point_Chart');
    ...
    if($request->hasFile('ancelled_cheque')){
    $ancelled_cheque =$request->file('ancelled_cheque');
    ...
    if($request->hasFile('Teleport_operator_certificate')){
       $Teleport_op_cert =$request->file('Teleport_operator_certificate');
    ...
    if($request->hasFile('Last_year_certificate')){
        $Last_year_certificate =$request->file('Last_year_certificate');
...
$upload_year_certificate =$Last_year_certificate->move($destinationPath,$Last_year_cr_re);
    ...
    if($request->hasFile('letter_attested')){
    $letter_attested =$request->file('letter_attested');
    ...
    if($request->hasFile('Government_India')){
    $Government_India =$request->file('Government_India');
    ...
    if($request->hasFile('applicant_channel_belongs')){
    $appl_chnel_belongs =$request->file('applicant_channel_belongs');
    ...
    if(!empty($request->HO_same_as_DO)){
      ...
      //Modification update
    if($Up_Down_lig_re != '' && $affirm ==1 && $Government_India_re !='' && $letter_attested_re !='' && $Last_year_cr_re !='' && $applicant_appl_chnel_re !='' && $Teleport_operator_certificate_re !='' && $ancelled_cheque_re !='' && $Fixed_Point_Chart_re !=''){
        $modification = 1;
    }else{
        $modification = 0;
    }
    ...
    $insert =array(
      ...
      "Global Dimension 2 Code"=>0,
       ...
       'Streaming Start Date' =>'1900-01-01 00:00:00.000',
       'Rate 12-17'=>0,
       "Rate 17-23"=>0,
       "Rate 7-9" =>0,
       "Rate 9-12"=>0,
    ...
    $msg ='';
    $Chanal_Details =DB::table('BOC$Vendor Emp - Channel')->where('User ID',$UserID)->first(); 
    $OwnerDetails =DB::table('BOC$Owner')
