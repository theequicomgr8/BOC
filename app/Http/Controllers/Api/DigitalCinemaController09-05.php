...
class DigitalCinemaController extends Controller
{
  use CommonTrait;
    ...
    private $vendorTable ='BOC$Vend Emp Digital Cinema';
...
/* ==================================start Owner Insert Update======================================*/
    ...
    {
        $table1 = '[BOC$Vend Emp Digital Cinema]';
        $msg = '';
    ...
    $userid = session::get('UserID');
         $agencyFind=DB::table($this->vendorTable)
            ->select('Agency Code')->where('User ID',$userid)->first();
                if(count($request->agency_contract_details) > 0){
                     $where2=array("Agency Code"=>@$agencyFind->{'Agency Code'});
                    $checkData=DB::table('BOC$Digital Cinema Screen')->where($where2)->delete();
                foreach ($request->agency_contract_details as $key => $value) {
                    $where=array("Agency Code"=>@$agencyFind->{'Agency Code'});
                    $line=DB::table('BOC$Digital Cinema Screen')->select('Line No_')->where($where)->orderBy('Line No_','desc')->first();
                    ...
                    $agency_contract_details =$value ?? '';
                    $screen_unique_code =$request->screen_unique_code[$key] ?? '';
                    ...
                    $insert =[
                      ...
                      "No_ Of Seats" =>$number_screens
                         ...
                         $sql1 =DB::table('BOC$Digital Cinema Screen')
                        ->insert( $insert);
...
public function ShowDetails($userID =''){
        $screentable ='BOC$Digital Cinema Screen';
