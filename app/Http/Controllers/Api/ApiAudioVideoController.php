...
class ApiAudioVideoController extends Controller
{
    ...
    public static function ftab_insert($request)
    {
        ...
        $tab1 =DB::table('BOC$Vend Emp - AV Producer')->select('AV Producer ID')->where('User ID',$user_id)->first();
       //dd($tab1);
        if(is_null($tab1))
        {
            ...
            $dataid = DB::select('select TOP 1 [AV Producer ID] from dbo.[BOC$Vend Emp - AV Producer] order by [AV Producer ID] desc');
            if (empty($dataid)) {
                $id = 'AV001';
            } else {
                $id = $dataid[0]->{"AV Producer ID"};
                $id++;
            }
            ...
            $sql=DB::table('BOC$Vend Emp - AV Producer')->insert($insert_array);
               if($sql)
            {
        ...
        else
        {
            ...
            $PersonBioData = 0;
            $Org_LegalStatus = 0;
            if($request->hasfile('income_tax_return'))
            {
                $file = $request->file('income_tax_return');
            ...
            $sql =  DB::update("UPDATE $table set
                ...
                [Org_ Legal Status] =$Org_LegalStatus
    ...
    {
        $user_id = Session('UserID');
        $av_id = DB::table('BOC$Vend Emp - AV Producer')->select('AV Producer ID')->where('User ID',$user_id)->first();
    ...
    {
        $data = DB::table('BOC$Vend Emp - AV Producer')->select('*')->where('User ID',$user_id)->first();
        if($data){
