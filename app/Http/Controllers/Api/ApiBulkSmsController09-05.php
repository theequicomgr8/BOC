...
class ApiBulkSmsController extends Controller
{
	...
	use CommonTrait;
  private $vendorTable='BOC$Vend Emp Bulk SMS';

	public function bulkSmssaveData(Request $request)
	{
		$table='[BOC$Vend Emp Bulk SMS]';
		$msg= '';
    ...
    if($request->hasFile('GST_Upload_File_Name')){
      ...
      if(!empty($gst_DB)){
    	...
    	if (@$Agency_id->{'Agency Code'} == '') {
            $Agency_Code = DB::select('select TOP 1 [Agency Code] from dbo.[BOC$Vend Emp Bulk SMS] order by [Agency Code] desc');
            if (empty($Agency_Code)) {
                $Agency_Code = 'AG00001'; // New Create Agency Code
            } else {
