...
class SoleRightMediaController extends Controller
{
    ...
    public function InsertsoleRightMedia(Request $request)
    {
        ...
        if ($url == 'sole-right-media') {
            ...
            $odmedia = DB::table('BOC$OD Media Owner Detail')->where($where)->where("Owner ID", "!=", '')->orderBy('OD Media ID', 'desc')->first();
            @$mediaid = $odmedia->{'OD Media ID'};

            $where2 = array("OD Media ID" => $mediaid, "OD Media Type" => 0);
            $authorize = DB::table('BOC$OD Auth Representative')->where($where2)->get();
            $branch = DB::table('BOC$OD Branch Offices')->where($where2)->get();

            // Lat Long code start

            $latlongData = DB::table('BOC$OD Latlong Detail')->select("Latitude", "Longitude", "Image File Name", "Remarks", "Far Image File Name", "City", "Near Picture", "Far Picture", "Tag Name")->where('OD Vendor ID', $mediaid)->get();
            ...
            $getcat = DB::table('BOC$OD Media Category')->select('OD Media UID as media_uid', 'Name as name')->get();
            ...
            if (@$vendor_data[0]->{'Modification'} == 0) {
                //for all category display sk
                $getcat = DB::table('BOC$OD Media Category')->select('OD Media UID as media_uid', 'Name as name')->where('Category Group', @$OD_media_address_data[0]['OD Media Type'])->get();
                return view('admin.pages.sole-right-media-form', ['latlongData' => $latlongData, 'states' => $states['original']['data'], 'ownerDistricts' => $ownerDistricts['original']['data'], 'ownerCities' => $ownerCities['original']['data']])->with(compact('owner_data', 'vendor_data', 'media_owner_details', 'OD_work_dones_data', 'OD_media_address_data', 'getcat', 'authorize', 'branch'));
            } else {
                $branch = array();
                $authorize = array();
                $latlongData = array();
                return view('admin.pages.sole-right-media-form', ['states' => $states['original']['data'], 'ownerDistricts' => $ownerDistricts['original']['data'], 'ownerCities' => $ownerCities['original']['data']])->with(compact('getcat', 'authorize', 'branch'));
            }
        } else {
            ...
            $find_owner_detail = DB::table('BOC$OD Media Owner Detail')->select('Owner ID')->where($where_detail)->first();
            ...
            //9 feb for find od media id through owner id
            $where = array("Owner ID" => $owner_id, "OD Media Type" => 0);
            $odmedia = DB::table('BOC$OD Media Owner Detail')->where($where)->orderBy('OD Media ID', 'desc')->first();
    ...
    {
        return view('admin.pages.vendor-sole-right-media-form');
    }
    ...
    public function viewSoleRightMedia($mediaid = '')
    {
        ...
        if (!empty($mediaid)) {
            ...
            return view(
                'admin.pages.view-sole-right-media',
                compact(
                    'owner_data',
                    'vendor_data',
                    'OD_work_dones_data',
    ...
    public function fileupload(Request $request)
    {
        ...
        if ($up) {
            ...
            if ($length == $total_media) {
                ...
                return "Your data have been submitted successfully. " . $odmedia_id;
            }
            // else
            // {
            //     $da=array(
    ...
    public function solerightRenewalView(Request $request)
    {
        ...
        //old code start
        // $userid = Session::get('UserID'); //comment by me 29-Dec sk
        ...
        //comment by me 29Dec old data sk
        // $data = (new solerightMedapi)->showDetails($userid);
        // $response = json_decode(json_encode($data), true);
    ...
    public function ownerRenewal(Request $request)
    {
        ...
        if (!empty($ownerData)) {
            ...
            DB::table('BOC$Owner')->where('Email ID', $email)->update($update);
    ...
    public function solerightRenewall(Request $request)
    {
        ...
        if ($getID == 0) {
            ...
            DB::table('BOC$OD Vendor Renewal')->insert($insertArray); //Insert Query In Vendor renewal table 30-Dec Sk
            ...
            if (count($request->ODMFO_Year) > 0) {
                ...
                if ($linenn_no > 0) {
                    DB::table('BOC$OD Media Work done')->where('OD Media ID', $odmedia_id)->where('OD Media Type', 0)->delete();
                }
                ...
                foreach ($request->ODMFO_Year as $key => $value) { //ram code start
                    ...
                    //Find Last Line No_ by "OD Media ID" in "BOC$OD Media Work done" table 31-Dec 
                    ...
                    DB::table('BOC$OD Media Work done')->insert([
                        "OD Media Type" => 0,
        ...
        } else //update query
        {
            ...
            if (@$check_status->Status != "6") {
                ...
                DB::table('BOC$OD Vendor Renewal')->insert($againInsert);
            }
    ...
    public function MediaWorkDone_delete(Request $request)
    {
        ...
        $data = DB::table('BOC$OD Media Work done')->where($where)->delete();
    ...
    {
        $code = $request->code;
        $res = DB::table('BOC$Owner')->select('Owner Name as name', 'Mobile No_ as mobile', 'Email ID as email', 'Address 1 as address', 'City', 'State', 'District', 'Phone No_ as phone_no')->where('Owner ID', $code)->first();
        //  dd($res);
        // return $res->name;
        if (!empty($res)) {
            // return $res;
    ...
    public function solerightlist()
    {
        ...
        // $vendor = DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID as media_id', 'HO E-Mail as ho_email', 'GST No_ as gst', 'HO Mobile No_ as mobile', 'Modification', 'Allocated Vendor Code as vendor_code', 'License From as from_date', 'License To as to_date')->where($where)->get();
        $vendor = DB::table('BOC$Vendor Emp - OD Media as odmedia')
                    ...
                    ->leftJoin('BOC$OD Vendor Renewal as renewal', "renewal.OD Media ID", "=", "odmedia.OD Media ID")
        ...
        foreach ($vendor as $key => $val) {
            $payment_status = DB::table('BOC$Vendor Fees')->select('Status')->where('Media ID', $val->media_id)->first();
        ...
        return view('admin.pages.sole-right-list', ["vendor" => $vendor, 'pending_count' => $pending_count]);
    }

    public function solerightlist3()
    {
        $userid = Session::get('UserID');
        $where = array('User ID' => $userid, "Modification" => 1);
        $vendor = DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID as media_id', 'HO E-Mail as ho_email', 'GST No_ as gst', 'HO Mobile No_ as mobile', 'Modification', 'Allocated Vendor Code as vendor_code', 'License From as from_date', 'License From as to_date')->where($where)->get();
        // dd($vendor[0]->media_id);
        // dd($vendor);
        return view('admin.pages.sole-right-list', ["vendor" => $vendor]);
    }


    public function solerightlist2()
    {
        $userid = Session::get('UserID');
        // ,'sra.OD Media ID as sub_category2'   'odv.HO E-Mail as ho_email','sma.OD Media Type'
        $vendor = DB::table('BOC$Vendor Emp - OD Media as odv')
            ->Join('BOC$Sole Medias Address as sma', 'sma.Sole Media ID', '=', 'odv.OD Media ID')

            ->join('BOC$OD Media Category as smc', 'smc.OD Media UID', '=', 'sma.OD Media ID')

            ->select(
                'odv.OD Media ID as mediaID',
                DB::raw("string_agg(smc.Name,',') as name")
            )
            // ->select(
            // 'odv.OD Media ID as media_id', 'odv.HO E-Mail as ho_email','sma.OD Media Type','smc.Name')   

            ->groupBy('odv.OD Media ID')
            ->where(['User ID' => $userid, "Status" => 1, 'OD Category' => 0])
            ->get();
    ...
    //     $vendor=DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID')->where($where)->first();
    //     if($vendor)
    //     {
    //         $od_media_id=$vendor->{'OD Media ID'};
    //         $wh=array("Sole Media ID"=>$od_media_id,"OD Media ID"=>$subcat);
    //         $sole=DB::table('BOC$Sole Medias Address')->select('OD Media ID')->where($wh)->first(); 
    //         // dd($sole);
    //         if($sole)
    //         {
    //             // return "Yes";
    ...
    public function findSubCategory(Request $request)
    {
        ...
        $vendor = DB::table('BOC$Vendor Emp - OD Media')->where($where)->pluck('OD Media ID')->toArray();
        if ($vendor) {
            $od_media_id = implode(",", $vendor);
            // $od_media_id=$vendor->{'OD Media ID'};
            $sole = DB::table('BOC$Sole Medias Address')->select('OD Media ID')->where("OD Media ID", $subcat)->whereIn("Sole Media ID", [$vendor])->get();
    ...
    public function show_subcategory(Request $request)
    {
        ...
        $data = DB::table('BOC$Sole Medias Address')->select('OD Media Type as cat', 'OD Media ID as subcat')->where('Sole Media ID', $cat_id)->get();
        $cat_temp = [];
        $sub_temp = [];
        $temp1 = [];
        foreach ($data as $list) {
            $cat_data = DB::table('BOC$OD Media Category')->select('*')->where("OD Media UID", $list->subcat)->get();
    ...
    public function show_category(Request $request)
    {
        ...
        foreach ($data as $list) {
            $cat_temp[] = $list->cat;
            // $cat_data=DB::table('BOC$OD Media Category')->select('*')->where("OD Media Type",$list->cat)->get();          
            // foreach($cat_data as $cat_list)
            // {
    ...
    public function existinguser()
    {
        ...
        $vendor = DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID" => $user_id, "OD Category" => 0, "Modification" => 0])->first();
        if ($vendor) {
            $select = array(
                'OD Media Type',
                'Sole Media ID',
            ...
            $OD_media_address_datas = DB::table('BOC$Sole Medias Address')->select($select)->where("Sole Media ID", $odmediaid)->get();
            $work = DB::table('BOC$OD Media Work done')->select("*")->where(["OD Media ID" => $odmediaid, "OD Media Type" => 0])->get();
            $getcat = DB::table('BOC$OD Media Category')->select('OD Media UID as media_uid', 'Name as name')->where('Category Group', $OD_media_address_datas[0]->{'OD Media Type'})->get();
            $vendorcheck = 0;
        ...
        return view('admin.pages.sole-right-existing-form', ['states' => $states['original']['data'], 'districts' => $districts['original']['data']], compact('vendor', 'OD_media_address_data', 'work', 'getcat', 'vendorcheck'));
    }
    ...
    public function existingUserSaved(Request $request)
    {
        $user_id = Session::get('UserID');
        // $user_id='EMRVV76';
        $ownerdata = DB::table('BOC$Owner')->select("*")->where('User ID', $user_id)->first();
        $ownerid = $ownerdata->{'Owner ID'};

        $vendordata = DB::table('BOC$Vendor Emp - OD Media')->select("*")->where(["User ID" => $user_id, "OD Category" => 0])->orderBy('OD Media ID', 'desc')->first();
        ...
        $mod = DB::table('BOC$Vendor Emp - OD Media')->where(["User ID" => $user_id, "OD Category" => 0, "Modification" => 0])->first();
        ...
        if ($request->vendorcheck == 1) {
            ...
            $odmedia_id_check = DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID')->orderBy('OD Media ID', 'desc')->first();
            ...
            $getBranch = DB::table('BOC$OD Branch Offices')->where(["OD Media ID" => $old_odmedia_id, "OD Media Type" => 0])->get();
            // $branch_ary=[];
            foreach ($getBranch as $key => $getBranchs) {
                $line_no = DB::table('BOC$OD Branch Offices')->select('Line No_')->where('OD Media ID', $odmedia_id)->orderBy('Line No_', 'desc')->first();
                if (empty($line_no)) {
                    $line_no = 10000;
                } else {
                    $line_no = $line_no->{"Line No_"};
                ...
                $branch = DB::table('BOC$OD Branch Offices')->insert([
                    "OD Media Type" => 0,
                    "OD Media ID" => $odmedia_id,
                    "Line No_" => $line_no,
            ...
            $getAuth = DB::table('BOC$OD Auth Representative')->where(["OD Media ID" => $old_odmedia_id, "OD Media Type" => 0])->get();
            foreach ($getAuth as $key => $getAuths) {
                $line_no = DB::table('BOC$OD Auth Representative')->select('Line No_')->where('OD Media ID', $odmedia_id)->orderBy('Line No_', 'desc')->first();
                if (empty($line_no)) {
                    $line_no = 10000;
                } else {
                    $line_no = $line_no->{"Line No_"};
                ...
                $auth = DB::table('BOC$OD Auth Representative')->insert([
                    "OD Media Type" => 0,
                    "OD Media ID" => $odmedia_id,
                    "Line No_" => $line_no,
            ...
            //     $aut=DB::table('BOC$OD Auth Representative')->insert($authorized_data);
            // }
            // if (!$aut) {
            ...
            $vendorinsert = DB::table('BOC$Vendor Emp - OD Media')->insert($vendor_array);

            $detail_data = DB::table('BOC$OD Media Owner Detail')->insert([
                "OD Media Type" => 0,
                "OD Media ID" => $odmedia_id,
                "Owner ID" => $ownerid,
            ...
            if ($length == $total_media) {
                ...
                DB::table('BOC$Vendor Emp - OD Media')->where('OD Media ID', $odmedia_id)->update($da);
            ...
            $vend = DB::table('BOC$Vendor Emp - OD Media')->select("*")->where(["User ID" => $user_id, "OD Category" => 0, "Modification" => 0])->orderBy('OD Media ID', 'desc')->first();
            ...
            $vendorUpdate = DB::table('BOC$Vendor Emp - OD Media')->where(["OD Media ID" => $odmediaid, "OD Category" => 0, "Modification" => 0])->update($vendor_array);

            $check = DB::table('BOC$Sole Medias Address')->select('*')->where('Sole Media ID', $odmediaid)->first();
            if ($check) {
                DB::table('BOC$Sole Medias Address')->where('Sole Media ID', $odmediaid)->delete();
            }
                ...
                if (!empty($request->MA_City[0])) {
                    foreach ($request->MA_City as $key => $value) {
                        ...
                        if (!empty($request->MA_City[0])) {
                            ...
                            $table55 = '[BOC$Sole Medias Address]';
                            //$check=DB::table('BOC$Sole Medias Address')->where('Sole Media ID',$odmediaid)->first();
                            // if($check)
                            // {
                            //     DB::table('BOC$Sole Medias Address')->where('Sole Media ID',$odmediaid)->delete();
                            // }
            ...
            $check = DB::table('BOC$OD Media Work done')->where('OD Media ID', $odmediaid)->first();
            if ($check) {
                DB::table('BOC$OD Media Work done')->where('OD Media ID', $odmediaid)->delete();
            }
            $table4 = '[BOC$OD Media Work done]';
            // if (count($request->ODMFO_Year) > 0) {
            $od_media_category = 0;
            ...
            if ($length == $total_media) {
                ...
                DB::table('BOC$Vendor Emp - OD Media')->where('OD Media ID', $odmediaid)->update($da);
    ...
    public function sole_right_renewal_form($id = '')
    {
        if ($id != '') {
            ...
            $getODmediaID = DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID" => $user_id, "OD Category" => 0, "Allocated Vendor Code" => $id])->first();
            $odmedia_id = @$getODmediaID->{'OD Media ID'};

            $check_data_in_renewal_table = DB::table('BOC$OD Vendor Renewal')->where(["User ID" => $user_id, "OD Category" => 0, "OD Media ID" => $odmedia_id])->first();
            $select = array(
                'OD Media Type',
                'Sole Media ID',
                'Line No_',
            ...
            if ($check_data_in_renewal_table == '' || $check_data_in_renewal_table == null || $check_data_in_renewal_table == 'null') {
                $vendor = DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID" => $user_id, "OD Category" => 0, "Modification" => 1, "OD Media ID" => $odmedia_id])->first();
                if ($vendor) {

                    $odmediaid = $vendor->{'OD Media ID'};
                    $soleaddress = DB::table('BOC$Sole Medias Address')->select($select)->where("Sole Media ID", $odmediaid)->get();
                    $work = DB::table('BOC$OD Media Work done')->select("*")->where(["OD Media ID" => $odmediaid, "OD Media Type" => 0])->get();
                    $vendorcheck = 0;
                ...
                return view('admin.pages.sole-right-renewal-form', ['states' => $states['original']['data'], 'districts' => $districts['original']['data']], compact('vendor', 'OD_media_address_data', 'work', 'getcat', 'vendorcheck', 'disabled', 'pointer', 'modification_renew'));
            } else {
                $vendor = DB::table('BOC$OD Vendor Renewal')->select('*')->where(["User ID" => $user_id, "OD Category" => 0, "OD Media ID" => $odmedia_id])->orderBy('Line No_', 'desc')->first();
                if ($vendor) {
                    $odmediaid = $vendor->{'OD Media ID'};
                    $soleaddress = DB::table('BOC$Sole Medias Address')->select($select)->where("Sole Media ID", $odmediaid)->get();
                    $work = DB::table('BOC$OD Media Work done')->select("*")->where(["OD Media ID" => $odmediaid, "OD Media Type" => 0])->get();
    ...
    public function sole_right_renewal_form_submit(Request $request)
    {
        ...
        $getODmediaID = DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID" => $user_id, "OD Category" => 0, "Allocated Vendor Code" => $vendor_code])->first();
        $odmediaid = @$getODmediaID->{'OD Media ID'};

        $destinationPath = public_path() . '/uploads/sole-right-media/';
        $mod = DB::table('BOC$Vendor Emp - OD Media')->where(["User ID" => $user_id, "OD Category" => 0, "OD Media ID" => $odmediaid])->first();
        // dd($mod->{'Legal Doc File Name'});
        $Legal_Doc_File_Name = $mod->{'Legal Doc File Name'} ?? '';
        if ($request->hasFile('Legal_Doc_File_Name') || $request->hasFile('Legal_Doc_File_Name_modify')) {
            $file = $request->file('Legal_Doc_File_Name') ?? $request->file('Legal_Doc_File_Name_modify');
        ...
        $table4 = '[BOC$Vendor Emp - OD Media]';
        // $line_no = DB::select("select TOP 1 [Line No_] from $table4 where [OD Media ID] = '" . $odmediaid . "' order by [Line No_] desc");
        $line_no = DB::table('BOC$OD Vendor Renewal')->select('Line No_')->where('OD Media ID', $odmediaid)->orderBy('Line No_', 'desc')->first();
        if (empty($line_no)) {
            $line_no = 10000;
        } else {
            $line_no = $line_no->{"Line No_"};
        ...
        $renewalsave = DB::table('BOC$OD Vendor Renewal')->insert($vendor_renewal);
        ...
        if (count($request->MA_City) > 0) {
            foreach ($request->MA_City as $key => $value) {
                ...
                if (!empty($request->MA_City[0])) {
                    ...
                    //     DB::table('BOC$Sole Medias Address')->where('Sole Media ID',$odmediaid)->delete();
                    // }
                    $sql5 = DB::table('BOC$Sole Medias Address')->insert([
                        "OD Media Type" => $Applying_For_OD_Media_Type,
        ...
        if (!empty($request->ODMFO_Quantity_Of_Display_Or_Duration[0])) {
            ...
            foreach ($request->ODMFO_Year as $key => $value) {
                ...
                $sql4 = DB::table('BOC$OD Media Work done')->insert([
    ...
    public function accountDetails()
    {
        ...
        return view('admin.pages.soleright.account_details', compact('data'));
    }
     ...
     public function outdoorsoleRightPdf() 
    {
        $userid = Session::get('UserID');
        $where = array('User ID' => $userid, "Modification" => 1, 'OD Category' => 0);
        $solepdfdata = DB::table('BOC$Vendor Emp - OD Media')
        ->select('OD Media ID as media_id', 'HO E-Mail as ho_email', 'GST No_ as gst', 'HO Mobile No_ as mobile', 'Modification', 'Allocated Vendor Code as vendor_code', 'License From as from_date', 'License To as to_date')
        ->where($where)
        ->get();
    ...
    {
        $res = DB::table('BOC$Trains')->select('Train No_ as train_no', 'Name as name')
