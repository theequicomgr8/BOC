...
class PersonalMediaController extends Controller
{
    ...
    public function InsertpersonalMedia()
    {
        if (!Session::has('id')) {
            ...
            if($url=="rate-settlement-personal-media")
            {
                ...
                $check=DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID"=>$userID,"OD Category"=>1,"Modification"=>0])->orderBy('OD Media ID','desc')->first(); 
                if(!empty($check))
                {
                ...
                $soleaddress=DB::table('BOC$Sole Medias Address')->select("*")->where("Sole Media ID",$mediaid)->get(); //for media category
                //for all category display sk
                $getcat = DB::table('BOC$OD Media Category')->select('OD Media UID as media_uid', 'Name as name')->get();
                // $where = array('OD Category' => 1, 'User ID' => $userID);


                $data = (new personalmedapi)->showDetails($where, 'BOC$Vendor Emp - OD Media');
                ...
                if (@$vendor_data[0]['Modification'] == 0) {
                    ...
                    return view('admin.pages.personal-outdoor.personal-media-form', ['states' => $states['original']['data']])->with(compact('owner_data', 'vendor_data', 'OD_work_dones_data', 'OD_media_address_data', 'mediaSubCatData','latlongData','branch','soleaddress','getcat'));
                }

                $branch=[];
                $soleaddress=[];
                return view('admin.pages.personal-outdoor.personal-media-form', ['states' => $states['original']['data'],'latlongData'=>$latlongData])->with(compact('branch','soleaddress','getcat'));
            }
            else
            {
                $url=last(request()->segments());
                ...
                $check=DB::table('BOC$Vendor Emp - OD Media')->select('*')->where("OD Media ID",$mediaid)->first(); 
                if(!empty($check))
                {
    ...
    public function personalRenewalView(Request $request)
    {
        ...
        $data = (new personalmedapi)->showDetails($where, 'BOC$OD Vendor Renewal'); 
        $response = json_decode(json_encode($data), true);
    ...
    public function personallist()
    {
        ...
        $vendor=DB::table('BOC$Vendor Emp - OD Media')->select('OD Media ID as media_id','HO E-Mail as ho_email','GST No_ as gst','HO Mobile No_ as mobile','Modification','Allocated Vendor Code as vendor_code','License From as from_date','License To as to_date','Media Sub Category as sub_category')->where($where)->get();
    ...
    {   
        ...
        $vendor=DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID"=>$user_id,"OD Category"=>1,"Modification"=>0])->first();
        if($vendor)
        {
            $odmediaid=$vendor->{'OD Media ID'};  
            $soleaddress=DB::table('BOC$Sole Medias Address')->select("*")->where("Sole Media ID",$odmediaid)->get();
            $work=DB::table('BOC$OD Media Work done')->select("*")->where(["OD Media ID"=>$odmediaid,"OD Media Type"=>1])->get();
            $getcat = DB::table('BOC$OD Media Category')->select('OD Media UID as media_uid', 'Name as name')->get();
            $vendorcheck=0;
        }
        else
        ...
        return view('admin.pages.personal-outdoor.personal-existing-form',['states' => $states['original']['data'], 'districts' => $districts['original']['data']],compact('vendor','soleaddress','work','getcat','vendorcheck'));
    }
    ...
    public function existingUserSaved(Request $request)
    {
        ...
        $vendordata=DB::table('BOC$Vendor Emp - OD Media')->select("*")->where(["User ID"=>$user_id,"OD Category"=>1])->orderBy('OD Media ID','desc')->first();
        ...
        $mod = DB::table('BOC$Vendor Emp - OD Media')->where(["User ID"=>$user_id,"OD Category"=>1,"Modification"=>0])->first();
        ...
        if($request->vendorcheck==1)
        {
            ...
            $getBranch=DB::table('BOC$OD Branch Offices')->where(["OD Media ID"=>$old_odmedia_id,"OD Media Type"=>1])->get();
            // $branch_ary=[];
            ...
            if($request->xls==0)
            {
                if(!empty($request->Applying_For_OD_Media_Type))
                {
                    foreach($request->Applying_For_OD_Media_Type as $key => $value)
                    {
                        ...
                        $line_no=DB::table('BOC$Sole Medias Address')->select('Line No_','Sole Media ID')->where("Sole Media ID",$odmedia_id)->orderBy('Line No_','desc')->first();
                        if (empty($line_no)) 
                        {
                            $line_no = 10000;
                        } else 
                        ...
                        DB::table('BOC$Sole Medias Address')->insert($soleArray);
                    }
                } 
            }
        ...
        {
            $vend=DB::table('BOC$Vendor Emp - OD Media')->select("*")->where(["User ID"=>$user_id,"OD Category"=>1,"Modification"=>0])->orderBy('OD Media ID','desc')->first();
            ...
            $vendorUpdate=DB::table('BOC$Vendor Emp - OD Media')->where(["OD Media ID"=>$odmediaid,"OD Category"=>1,"Modification"=>0])->update($vendor_array);
            ...
            if ($check) {
                ...
                if($request->xls==0)
                {
                    if(!empty($request->Applying_For_OD_Media_Type))
                    {
                        foreach ($request->Applying_For_OD_Media_Type as $key => $value) 
                        {
                            $line_no=DB::table('BOC$Sole Medias Address')->select('Line No_','Sole Media ID')->where("Sole Media ID",$odmediaid)->orderBy('Line No_','desc')->first();
                            if (empty($line_no)) 
                            {
                                $line_no = 10000;
                            } else 
                            ...
                            $sql5=DB::table('BOC$Sole Medias Address')->insert($soleArray);
    ...
    public function personal_right_renewal_form($id='')
    {
        if($id!='')
        {
            ...
            $getODmediaID=DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID"=>$user_id,"OD Category"=>1,"OD Media ID"=>$id])->first();
            $odmedia_id=@$getODmediaID->{'OD Media ID'};

            $check_data_in_renewal_table=DB::table('BOC$OD Vendor Renewal')->where(["User ID"=>$user_id,"OD Category"=>1,"OD Media ID"=>$odmedia_id])->first();

            if($check_data_in_renewal_table=='' || $check_data_in_renewal_table==null || $check_data_in_renewal_table=='null')
            {
                $vendor=DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID"=>$user_id,"OD Category"=>1,"Modification"=>1,"OD Media ID"=>$odmedia_id])->first();
                if($vendor)
                {
                    $odmediaid=$vendor->{'OD Media ID'};  
                    $soleaddress=DB::table('BOC$Sole Medias Address')->select("*")->where("Sole Media ID",$odmediaid)->get();
                ...
                $disabled = ''; 
                $pointer='';  
                return view('admin.pages.personal-outdoor.personal-renewal-form',['states' => $states['original']['data'], 'districts' => $districts['original']['data']],compact('vendor','soleaddress','work','getcat','vendorcheck','disabled','pointer'));
            }
            else
            {
                $vendor=DB::table('BOC$OD Vendor Renewal')->select('*')->where(["User ID"=>$user_id,"OD Category"=>1,"OD Media ID"=>$odmedia_id])->orderBy('Line No_','desc')->first();
                if($vendor)
                {
                    $odmediaid=$vendor->{'OD Media ID'};  
                    $soleaddress=DB::table('BOC$Sole Medias Address')->select("*")->where("Sole Media ID",$odmediaid)->get();
    ...
    public function personal_renewal_form_submit(Request $request)
    {
        ...
        $getODmediaID=DB::table('BOC$Vendor Emp - OD Media')->select('*')->where(["User ID"=>$user_id,"OD Category"=>1,"OD Media ID"=>$request->od_media_id])->first();
        $odmediaid=@$getODmediaID->{'OD Media ID'};

        $destinationPath = public_path() . '/uploads/personal-media/';
        $mod = DB::table('BOC$Vendor Emp - OD Media')->where(["User ID"=>$user_id,"OD Category"=>1,"OD Media ID"=>$odmediaid])->first();
        // dd($mod->{'Legal Doc File Name'});
        $Legal_Doc_File_Name = $mod->{'Legal Doc File Name'} ?? '';
        if ($request->hasFile('Legal_Doc_File_Name') || $request->hasFile('Legal_Doc_File_Name_modify')) {
            $file = $request->file('Legal_Doc_File_Name') ?? $request->file('Legal_Doc_File_Name_modify');
