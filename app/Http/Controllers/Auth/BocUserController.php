    ...
    public function createSignup(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (empty($Ummuser) && empty($Ummusergst)) {
                ...
                //     )values
                //     (DEFAULT, $userType, '" . $user_id . "', '" . $usrNewsCode . "', 0, '" . $password . "', '" . $email . "', '".$request->mobile."', '',  0, '', '" . $application_date . "','',0)");
                ...
                if ($save) {
                    ...
                    if ($res) {
                        ...
                        // $dat=["name"=>$request->email,"url"=>$url,"otp"=>$otp];
                        // // $dat=["name"=>$request->email,"url"=>$url];
                        // $user['to']=$request->email;
                        // Mail::send('emails.mail',$dat,function($msg) use($user){
                        //     $msg->to($user['to']);
                        //     $msg->subject('Testing');
                        // });
                        // Send mail
                        ...
                        // Session::flash('otp_message', 'Please check sms for OTP ' . $otp);
                ...
                if (($credentials->email == $request->email && $passwordMatch) || ($credentials->UserName == $request->email && $passwordMatch) || ($credentials->GST == $request->email && $passwordMatch) || ($credentials->UserID == $request->email && $passwordMatch)) {
                    if ($credentials->email_verification == 1 && $credentials->Active == 1) {
                        ...
                        {//get agency code
                            ...
                            elseif($wing==5)
                            {
                                $agency_type=0; //av radio
                            }
                            elseif($wing==7)
                            {
                                $agency_type=2; //av producer
                            }
                           $AVagencyData= DB::table('BOC$AV Vendor$3f88596c-e20d-438c-a694-309eb14559b2')->select('Agency Code as AgencyCode')->where('User ID',$credentials->UserID)->where('Agency Type',$agency_type)->first();
                           if(!empty($AVagencyData)){
                             Session::put('AgencyCode', $AVagencyData->AgencyCode);
                           }
   ...
   public function submitotp()
    ...
    public function changePassword(Request $request) {
       ...
       $get_pass = DB::table('BOC$UMM User$3f88596c-e20d-438c-a694-309eb14559b2')->select('password')->where('User ID',$userid)->first();
        ...
        if ($sql) {
            return back()->with(['status' => true, 'message' =>  'Password successfully Changes']);
        } else {
            return back()->with(['status' => false, 'message' => 'Failed']);
        }
